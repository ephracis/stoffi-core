...
namespace Stoffi.Core.Plugins
{
	...
	public static class Plugins
	{
		...
		static Plugins()
		{
			if (Settings.Manager.Plugins == null)
				Settings.Manager.Plugins = new ObservableCollection<PluginItem>();

			Settings.Manager.Plugins.Clear();
			...
			Settings.Manager.Plugins.CollectionChanged += new NotifyCollectionChangedEventHandler(Plugins_CollectionChanged);
		...
		public static void Initialize()
		{
            Clean();
			Install();
			ActivateVisualizer(Settings.Manager.CurrentVisualizer);
		}
						...
						foreach (Type type in types)
						{
							...
							if (plugin != null)
							{
								...
								if (plugin.Type == PluginType.Visualizer || plugin.Type == PluginType.Filter)
								{
									...
									plugin.CurrentCulture = Settings.Manager.Culture.IetfLanguageTag;
									Install(plugin);
								}
							}
						}
		...
		public static void Install(Plugin plugin)
		{
			if (plugin.OnInstall())
			{
                ...
                Metadata set = GetSettings(plugin.ID);
                if (set != null)
                {
                ...
                if (set == null)
                {
					Settings.Manager.PluginSettings.Add(new Metadata
                    {
                        PluginID = plugin.ID,
		...
		public static void Start(Plugin plugin)
		{
			if (!activePlugins.Contains(plugin))
			{
				if (plugin.OnStart())
				{
					...
					{
						var filter = plugin as Stoffi.Plugins.Filter;
		...
		public static void Uninstall(Plugin plugin)
		{
			...
			// forget settings
			for (int i=0; i < Settings.Manager.PluginSettings.Count; i++)
			{
				if (Settings.Manager.PluginSettings[i].PluginID == plugin.ID)
				{
					Settings.Manager.PluginSettings.RemoveAt(i);
					break;
				}
			}
		...
		{
			foreach (PluginItem p in Settings.Manager.Plugins)
                if (p.ID == id)
					return p;
			return null;
		}

		public static Metadata GetSettings(string id)
        {
			foreach (Metadata s in Settings.Manager.PluginSettings)
                if (s.PluginID == id)
                    return s;
            return null;
        }
		...
		private static void ActivateVisualizer(Visualizer visualizer = null)
		{
			foreach (Plugin p in plugins)
			{
				if (p.Type == PluginType.Visualizer &&
                    p.ID == Settings.Manager.CurrentVisualizer)
					Start(p);
					...
					foreach (Plugin p in activePlugins)
					{
						switch (p.Type)
						{
							case PluginType.Visualizer:
								var v = p as Stoffi.Plugins.Visualizer;
								Media.Manager.FFTData.CopyTo(v.FFTData, 0);
								DispatchRefresh(v);
								break;
		...
		/// <param name="sender">The sender of the event</param>
		/// <param name="e">The event data</param>
		private static void Plugin_VolumeChanged(object sender, Stoffi.Plugins.GenericEventArgs<double> e)
		{
			Settings.Manager.Volume = e.Value;
		}
	...
	/// </summary>
	public class PluginItem : ListItem
	{
		#region Members
