 ...
 * OAuth.cs
 * 
 * Code to do OAuth stuff, in support of a cropper plugin that
 * sends a screen snap to TwitPic.com.
 *
 * There's one main class: OAuth.  It handles interaction
 * with the OAuth-enabled service, for requesting temporary tokens
 * (aka request tokens), as well as access tokens. It also provides
 * a convenient way to construct an oauth Authorization header for
 * use in any Http transaction.
 *
 * The code has been tested with Twitter and TwitPic, from a
 * desktop application.
 * 
 * * * * * * * * *
 * 
 * Copyright 2013 Dino Chiesa, Simplare
 * 
 * This code was taken from TwitPic (www.twitpic.com) with
 * permission and is part of the Stoffi Music Player Project.
		...
		public static string UrlEncode(string value)
		{
			...
			foreach (char symbol in value)
			{
				if (unreservedChars.IndexOf(symbol) != -1)
					...
					result.Append(String.Format("%26%23{0}%3B", (int)symbol));
				else
