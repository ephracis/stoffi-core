...
namespace Stoffi.Core.Upgrade
{
	...
	public static class Manager
	{
		...
		private static TimeSpan probeInterval;
		private static long probeDelay;
		...
		/// </summary>
		public static Settings.UpgradePolicy Policy
		{
			get { return Settings.Manager.UpgradePolicy; }
			set { Settings.Manager.UpgradePolicy = value; }
		}
		...
		/// </summary>
		/// <param name="interval">How often to check for new upgrades</param>
		/// <param name="delay">The delay before performing the first check (in seconds)</param>
		public static void Initialize(TimeSpan interval, long delay)
		{
			probeInterval = interval;
			probeDelay = delay*1000;
			Pending = false;
			ForceDownload = false;
			InProgress = false;
...
#else
			Stop();
			if (Settings.UpgradePolicy != UpgradePolicy.Manual)
				prober = new Timer(Probe, null, probeDelay, (long)probeInterval.TotalMilliseconds);
#endif
		}
		...
		public static void Probe(object state)
		{
...
#if (DEBUG)
			U.L(LogLevel.Warning, "UPGRADE", "Running debug build, no upgrade checks");
			if (Settings.Manager.UpgradePolicy == UpgradePolicy.Manual)
			{
			...
			haveID = true;
			try { URL += String.Format("&id={0}", Settings.ID); }
			catch { haveID = false; }
			if (Settings.UpgradePolicy != UpgradePolicy.Automatic && !ForceDownload)
				URL += "&p=1";
			...
			else
			{
				...
				client.Headers.Add("User-Agent", String.Format("Stoffi/{0}", Settings.Version));
				...
				if (!haveID)
				{
					if (File.Exists(baseFolder + "/" + upgradeFolder + downloadFilename + "_tmp/outgoing/data"))
					{
						...
						while ((line = file.ReadLine()) != null)
						{
							...
							{
								Settings.Manager.ID = Convert.ToInt32(field[1].Trim());
					...
					try
					{
						while ((line = file.ReadLine()) != null) // TODO: check if closed?
						{
							...
							if (line.Length > 23 + 25 && (line.Substring(0, 23) == "No upgrade needed, mr. " && line.Substring(line.Length - 25, 25) == ". You're already awesome!"))
							{
								...
								Settings.Manager.ID = Convert.ToInt32(id);
								Settings.Manager.Save();
								file.Close();
								Clear();
		...
		/// 2) Optional: "Settings Migrator.dll", this will be used later to migrate the settings in Settings
		/// 
		...
		public static void Unpack()
		{
			...
			foreach (int version in versions)
			{
				...
				var c = Settings.Manager.Channel;
				var a = Settings.Manager.Architecture;

				var baseStr = String.Format("{0}/{1}{2}{3}/{4}/{5}", baseFolder, upgradeFolder, folder, c, a, version);
				var tarFile = baseStr + ".tar";
				var bz2File = tarFile + ".bz2";
				var tmpFold = baseStr + "_tmp/";
		...
		public static void Clear()
		{
			try
			{
				...
				var configFolder = Path.GetDirectoryName(Path.GetDirectoryName(config.FilePath));
				if (Directory.Exists(configFolder))
				{
					string[] dirs = Directory.GetDirectories(configFolder);
		...
		/// <param name="sender">The sender of the event</param>
		/// <param name="e">The event data</param>
		private static void Settings_PropertyChanged(object sender, Settings.PropertyChangedWithValuesEventArgs e)
		{
		...
		private static void DispatchChecked()
		{
			...
			Settings.Manager.UpgradeCheck = (long)_ts.TotalSeconds;
			if (Checked != null)
				Checked(null, null);
		}
